...
interface DebtsFormModalProps {
  ...
  initialData?: Partial<SupplierDebt> | null;
  isLoading?: boolean;
  currentUser?: unknown;
...
export function DebtsFormModal({
  ...
  // View Mode - Show all supplier debt information
  if (variant === "view" && initialData) {
    const debt = initialData as SupplierDebt;
    return (
      <Modal
        isOpen={isOpen}
        onClose={onClose}
        title={t("suppliers.debts.debtDetails")}
        maxWidth="max-w-2xl"
        ...
        <div className="space-y-6">
          ...
          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
              ...
              {t("suppliers.debts.supplierInformation")}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-muted-foreground">
                  {t("suppliers.debts.columns.supplier")}
                </label>
                <p className="text-sm font-medium">
                  {debt.supplier?.name ||
                    `${t("suppliers.debts.form.supplierId")}: ${
                      debt.supplierId
                    }`}
                </p>
              </div>
              <div>
                ...
                <p className="text-sm font-medium">{debt.supplierId}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
              ...
              {t("suppliers.debts.debtInformation")}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-muted-foreground">
                  {t("suppliers.debts.columns.amount")}
                </label>
                <p className="text-sm font-medium">
                  {formatCurrency(debt.amount ?? 0)}
                </p>
              ...
              <div>
                <label className="text-sm font-medium text-muted-foreground">
                  {t("suppliers.debts.columns.status")}
                </label>
                <div className="mt-1">{getStatusBadge()}</div>
              </div>
              <div>
                <label className="text-sm font-medium text-muted-foreground">
                  {t("suppliers.debts.form.transactionId")}
                </label>
                <p className="text-sm font-medium">
                  {debt.transactionId || t("suppliers.debts.notProvided")}
                </p>
              </div>
            </div>
          </div>
          ...
          {debt.alertSent !== undefined && (
            <div>
              <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
                ...
                {t("suppliers.debts.alertInformation")}
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-muted-foreground">
                    {t("suppliers.debts.columns.alertSent")}
                  </label>
                  <p className="text-sm font-medium">
          ...
          {debt.remarks && (
            <div>
              <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
                <FileText className="h-4 w-4" />
                {t("suppliers.debts.form.remarks")}
              </h4>
          ...
          <div>
            <h4 className="text-sm font-medium mb-3 flex items-center gap-2">
              ...
              {t("suppliers.debts.recordInformation")}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-muted-foreground">
  ...
  const getTitle = () => {
    switch (variant) {
      case "create":
        return t("suppliers.debts.createTitle");
      case "edit":
        return t("suppliers.debts.editTitle");
      default:
        return t("suppliers.debts.debtDetails");
    }
  };
  ...
  const getLoadingText = () => {
    switch (variant) {
      case "create":
        return t("suppliers.debts.creating");
      case "edit":
        return t("suppliers.debts.updating");
      default:
        return t("suppliers.debts.saving");
    }
  };
    ...
    <div className="space-y-3">
      <div>
        ...
        <Input
          ...
          placeholder={t("suppliers.debts.form.supplierIdPlaceholder")}
          required
        />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">
          {t("suppliers.debts.form.amount")}
        </label>
        <Input
          type="number"
          step="0.01"
          ...
          placeholder={t("suppliers.debts.form.amountPlaceholder")}
          required
        />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">
          {t("suppliers.debts.form.dueDate")}
        </label>
        <Input
          type="date"
          value={form.dueDate}
      ...
      <div>
        ...
        <Input
          ...
          placeholder={t("suppliers.debts.form.transactionIdPlaceholder")}
        />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">
        ...
        <Input
          ...
          placeholder={t("suppliers.debts.form.remarksPlaceholder")}
        />
      </div>
    </div>
  );
